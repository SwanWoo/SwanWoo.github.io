...
## 绿盟科技集团股份有限公司 — 安全服务工程师
**2025.05 - 2025.09 · 上海**

- 独立完成 20+ 套政府/金融/运营商对外业务系统的黑盒+灰盒渗透测试，覆盖 Web、API、移动 App 三类场景
- 累计发现 90+ 个有效漏洞（含 9 个高危 SQL 注入、3 个后台 Getshell）
- 参加 1 次国家级征信中心攻防演练，作为蓝方 28 天内累计等级研判 1800+ 条可疑数据
- 输出标准化报告与复测报告各 30 余份，报告一次通过率 100%
...
## 安全技能
- Web/API/移动应用渗透测试（黑盒 + 灰盒）
- SQL 注入、XSS、CSRF、SSRF 等常见漏洞挖掘与利用
- 攻防演练与应急响应

## AI Infra
- 大模型推理部署与加速（vLLM、KV Cache 管理、批处理调度）
